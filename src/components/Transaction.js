...
const Transaction = ({ transaction }) => {
    ...
    return (
        <li className="minus">
                     {transaction.text} <span>{moneySign}${Math.abs(transaction.amount)}</span> <button onClick={(event) => deleteTransaction(event, transaction.id)} className="delete-btn">X</button>
        </li>
    )
}
